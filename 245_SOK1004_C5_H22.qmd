...
df_wide74 <- df_wide74 %>%
  rename(bruttoprodukt = `Bruttoprodukt per utførte timeverk. Endring fra året før (prosent). Faste priser` ) %>%
  rename(timeverk = `Utførte timeverk for lønnstakere. Endring fra året før (prosent)`)
  
df_wide74 <- df_wide74%>%
mutate(arbeidsprod = bruttoprodukt - timeverk)
```

```{r}
df_wide1 %>%
  ggplot(aes(x = år, y = bruttoprodukt)) + 
  geom_col() +
  theme_bw() +
  scale_x_discrete(breaks=c(1996, 1998, 2000, 2002, 2004, 2006, 2008, 2010, 2012, 2014, 2016, 2018, 2020))
```
