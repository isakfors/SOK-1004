...
df_wide %>%
    filter(tid >=2000) %>%
  ggplot(aes(x=tid)) +
  geom_line(aes(y=Bruttonasjonalprodukt), color="dark red") +
  geom_line(aes(y=`MEMO: Bruttonasjonalprodukt. Faste 2015-priser`), color="steelblue") +
  scale_y_continuous(labels = scales::comma) +
  labs(title="Bruttonasjonalprodukt per kapita",
       x ="år",
       y = "verdi i kr") +
  theme_bw()
```
...
#Man kan se at ved å skille øknaden i bnp per kapira i faste priser og løpende priser får vi et bilde av hvordan valutaen påvirke vektsten. når prisen er fast ser man den reele veksten mens med løpende pris ser vi hvordan valutaen har økt sammens med bnp. Ved bruk av faste prisen får man et bedre bilde på veksten som man kan mer nøyaktig sammenligne med.
...
#I df er både bnp og bnp i faste priser i samme kolonne i tabellen mens i df_wide vil dataen bli videt(derfor pivot_wide) inn i to tabeller slik at man kan hente ut dataene separat.
...
```{r}
df_wide <- df_wide %>%
  mutate(DBNP = lag(Bruttonasjonalprodukt - LBNP,n=1L)) %>%
  mutate(DMEMOBNP = lag(`MEMO: Bruttonasjonalprodukt. Faste 2015-priser` - LMEMOBNP,n=1L))
mutate(lag)
...
```{r}
df_long <- df_wide %>%
    pivot_longer(c(`Bruttonasjonalprodukt`, `MEMO: Bruttonasjonalprodukt. Faste 2015-priser`, `LBNP`, `LMEMOBNP`, `DBNP`, `DMEMOBNP`), names_to = "var", values_to = "verdi")
```
...
#Ja! Vi kan obervere år med negativ vekst i reell bnp. Året 2008 skjedde finanskrisen som påvirket verdens finansielle systemer som også resulterte i en negativ vekst i den reelle bnp i norge. Året 2020 var det også negativ vekst på grunn av corona pandemien som rammet store deler av økonomien.
...
```{r}
df_wide %>%
  mutate(prosBNP = 100*(lag(DBNP))/lag(Bruttonasjonalprodukt)) %>%
  mutate(prosRBNP = 100*(lag(DMEMOBNP))/lag(`MEMO: Bruttonasjonalprodukt. Faste 2015-priser`)) %>%
  ggplot(aes(x=tid)) +
  geom_smooth(aes(y=prosBNP), color="dark red") +
  geom_smooth(aes(y=prosRBNP), color="steelblue") +
  labs(title="Prosentvis endring i BNP",
       x="år",
       y="prosent") +
  theme_bw()
```
